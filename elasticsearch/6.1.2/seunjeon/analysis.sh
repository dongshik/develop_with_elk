...
function testSeunjeon1 {
    ...
    $CURL -XPOST "${ES}/${es_idx}/doc/3?pretty" -d ' { "field1" : "아버지가 방에 들어간다." }' | jq
    $CURL -XGET "${ES}/${es_idx}/doc/3" | jq

    $CURL -XPOST "${ES}/${es_idx}/doc/4?pretty" -d ' { "field1" : "아버지께서 집에 들어간다." }' | jq
    $CURL -XGET "${ES}/${es_idx}/doc/4" | jq
